...
import { Upload, Loader2, History, Save, Truck, Plus, Trash2, Download, Receipt, X, FileText, TrendingUp, Wrench } from 'lucide-react';
...
export default function GestionPresupuestos() {
  ...
  const generarPDFCliente = (subtotal: number) => {
    ...
    // Epígrafe Mano de obra
    if (manoDeObra.coste > 0) {
      filasBase.push([
        `MANO DE OBRA${manoDeObra.descripcion ? ': ' + manoDeObra.descripcion.toUpperCase() : ''}`,
        '1',
        manoDeObra.coste.toLocaleString('es-ES', { minimumFractionDigits: 2 }),
        manoDeObra.coste.toLocaleString('es-ES', { minimumFractionDigits: 2 }),
      ]);
    }
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        2: { cellWidth: 38, halign: 'right' }, 3: { cellWidth: 38, halign: 'right', fontStyle: 'bold' },
      },
      ...
      didParseCell: (data) => {
        if (data.row.index < partidas.length) return;
        if (manoDeObra.coste > 0 && data.row.index === partidas.length) {
  ...
  const subtotalPartidas = partidas.reduce((a, p) => a + (Number(p.total_euros) || 0), 0);
  const subtotalActual = subtotalPartidas + (manoDeObra.coste || 0);
          ...
          {partidas.length > 0 ? (
            <div className="bg-white text-zinc-900 rounded-[2.5rem] shadow-2xl overflow-hidden border border-zinc-200">
              ...
              <div className="p-8">
                ...
                {/* Epígrafe Mano de obra */}
                <div className="mb-6 border-2 border-blue-100 rounded-2xl overflow-hidden">
                  <div className="bg-[#1e3d6b] px-6 py-3 flex items-center gap-2">
                    <Wrench size={16} className="text-white" />
                    <span className="text-[10px] font-black text-white uppercase tracking-widest">Epígrafe — Mano de Obra</span>
                  </div>
                  <div className="p-5 bg-blue-50 flex gap-4 items-end">
                    <div className="flex-1">
                      <label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-1">Descripción</label>
                      <input
                        className="w-full bg-white border border-blue-200 rounded-xl px-4 py-3 text-sm font-bold text-zinc-700 outline-none focus:border-[#1e3d6b]"
                        placeholder="ej. Montaje completo de tabiquería y falso techo..."
                        value={manoDeObra.descripcion}
                        onChange={e => setManoDeObra(prev => ({ ...prev, descripcion: e.target.value }))}
                      />
                    </div>
                    <div className="w-36">
                      ...
                      <input
                        ...
                        placeholder="0,00"
                        onChange={e => setManoDeObra(prev => ({ ...prev, coste: parseFloat(e.target.value) || 0 }))}
                      />
                    </div>
                  </div>
                </div>
