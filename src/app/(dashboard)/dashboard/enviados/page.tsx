...
import {
  ...
  FileText, Search, X, Edit2, CheckCircle2,
  Save, Loader2, Trash2, Download, Plus, RotateCcw
} from 'lucide-react';
...
export default function EnviadosPage() {
  ...
  const moverAPendientes = async () => {
    ...
    toast.success('Presupuesto movido a Pendientes ✓');
    setSeleccionado(null);
    fetchLista();
  };
    ...
    <div className="w-full max-w-[1200px] mx-auto p-6 lg:p-10 text-white">
              ...
              <div className="flex-1">
                ...
                {editandoNombre ? (
                  <div className="flex gap-2">
                    <input value={nuevoNombre} onChange={(e) => setNuevoNombre(e.target.value)} className="text-2xl font-black uppercase border-b-2 border-blue-500 outline-none w-full bg-transparent" autoFocus />
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <input className="bg-zinc-100 border border-zinc-200 rounded-lg px-3 py-1 text-sm font-black text-[#1e3d6b] outline-none focus:border-[#1e3d6b] w-28" value={numeroPresupuesto} onChange={(e) => setNumeroPresupuesto(e.target.value)} placeholder="26001" />
            ...
            <div className="p-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <button
                  ...
                  disabled={isMoving}
                  className="w-full bg-orange-500 hover:bg-orange-600 text-white p-4 rounded-2xl flex items-center justify-center gap-2 font-black uppercase text-xs shadow-lg active:scale-95 transition-all disabled:opacity-50"
                >
                  {isMoving ? <Loader2 className="animate-spin h-5 w-5" /> : <RotateCcw size={20} />}
                  Mover a Pendientes
                </button>
              </div>
      ...
      <div className="flex items-center justify-between mb-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tight">Presupuestos Enviados</h1>
            <p className="text-blue-100/60 text-sm">Presupuestos enviados al cliente. Puedes editarlos y volver a moverlos a Pendientes.</p>
          </div>
        </div>
        <Link href="/dashboard/pendientes" className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-xl text-xs font-black uppercase transition-all">
          ...
          {lista.map((p) => (
            <div key={p.id} onClick={() => abrirModal(p)} className="group bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-3xl flex flex-col md:flex-row justify-between items-center hover:bg-white/15 transition-all cursor-pointer shadow-lg">
              <div className="flex gap-6 items-center">
                <div className="p-4 rounded-2xl bg-green-500/20 text-green-300 shadow-inner">
                  <CheckCircle2 size={24} />
                </div>
                <div>
                  <h3 className="font-bold text-xl uppercase tracking-tighter group-hover:text-green-300 transition-colors">{p.obra_nombre}</h3>
                  <div className="flex gap-4 text-white/40 text-[11px] font-bold mt-1 uppercase tracking-widest">
                    <span className="flex items-center gap-1"><User size={12} className="text-green-300" /> {p.cliente_nombre}</span>
              ...
              <div className="text-right flex items-center gap-6">
                <div>
                  ...
                  <p className="text-[10px] font-black uppercase text-right tracking-widest text-green-400">ENVIADO</p>
                </div>
                <div className="p-4 bg-white/5 rounded-2xl group-hover:bg-green-500 transition-all group-hover:translate-x-1 shadow-inner">
